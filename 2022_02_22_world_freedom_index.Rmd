...
freedom %>%
  ...
  # geom_text(aes(label = region_name), vjust = 1, hjust = 1) +
  expand_limits(x = 0, y = 0, size = 0) #very nice to expand the scale to 0. 
...
freedom %>%
  filter(year == 2020) %>%
  gather(metric, value, civil_liberties, political_rights) %>% #this is to do like pivot_long or wide of specific columns:
  mutate(metric = str_to_title(str_replace_all(metric, "_", " ")), #this is to remove an underscore of the elements of that column and then to make the first letter capital. 
         region_name = fct_reorder(region_name, value)) %>% #here we reorder a column based on another column. 
