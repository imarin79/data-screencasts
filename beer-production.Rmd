...
brewing_materials <- readr::read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2020/2020-03-31/brewing_materials.csv') %>%
  mutate(date = ymd(paste(year, month, 1))) %>% #here the day is always 1, first day of the .With the ymd(), it treats each element based on its order: year, month, day and put and hyphen between them. 
  filter(year < 2016)
...
brewing_materials %>%
  filter(date == max(date)) %>% #here we just include the max date, which is 2015-12-01. 
  filter(!str_detect(material_type, "Total")) %>% #here we filter out every element in material_type that has the word "Total.
...
brewing_materials %>%
  ...
  labs(x = "Time",
       ...
       fill = "Material") #fill means the legend.

brewing_materials %>% count(material_type, sort = TRUE)

brewing_materials %>%
  filter(str_detect(material_type, "Total.*products")) %>% # .* indicate that the next word is products
  mutate(type = fct_reorder(type, month_current, sum)) %>% #sum is the function we can add to fct_reorder, so we can order type based on the sum of month_current variable.
  ggplot(aes(date, month_current, fill = type)) +
  geom_col() +
  scale_y_continuous(labels = scales::comma) +
  labs(x = "Time",
...
Tidymetrics and shinymetrics is a package to visualize interactive metrics overtime. 
```{r}
# devtools::install_github("ramnathv/tidymetrics")
library(tidymetrics)

brewing_summarized <- brewing_materials %>%
  rename(material = type) %>% #this is to change column names.
  filter(!str_detect(material_type, "Total")) %>%
